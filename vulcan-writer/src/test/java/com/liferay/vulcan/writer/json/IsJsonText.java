...
public class IsJsonText extends AbstractJsonElementMatcher<JsonPrimitive> {

  private final Matcher<? super String> textMatcher;

  private IsJsonText(final Matcher<? super String> textMatcher) {
    super(JsonElementType.STRING);
    this.textMatcher = Objects.requireNonNull(textMatcher);
  }
  public static Matcher<JsonPrimitive> jsonText(String text) {
    return new IsJsonText(is(text));
  }

  public static Matcher<JsonPrimitive> jsonText(Matcher<? super String> textMatcher) {
    return new IsJsonText(textMatcher);
  }

  @Override
  protected boolean matchesNode(JsonPrimitive node, Description mismatchDescription) {
    final String value = node.getAsString();
